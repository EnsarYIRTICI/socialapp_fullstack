...
function RoomScreen({ roomid, userData, ID_F }: RoomScreenProps) {
  ...
  useEffect(() => {
    ...
    socket.on(SocketKeys.ON_MESSAGE, () => fetchBubbleCount());
  ...
  const findBubblesMore = async () => {
    setLoading(true);
    if (bubbleList.length < countPrev) {
    ...
    >
      <div style={responsive.main}>
        ...
        <div
          ...
          onScroll={(e) => scrollDownButtonVisible(e)}
        >
          {bubbleList.map((bubbleData: BubbleData, i: number) => {
            return bubbleData.creation_date.substring(5, 10) !=
              ...
              <Bubble key={i} index={i} bubbleData={bubbleData} />
            );
          })}
        ...
        {photoSource && (
          <Dialog press={() => setPhotoSource(null)}>
            <PhotoDialog />
          </Dialog>
        )}
      </div>
